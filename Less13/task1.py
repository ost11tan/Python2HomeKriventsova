...
"""
from error import UserValueError
    ...
    def check(self, s: str) -> str:
        while True:
            ...
            if not tmp.isnumeric() or int(tmp) < 1 or int(tmp) > self.COLOMN:
                raise UserValueError(tmp, self.COLOMN)
                """Вопрос!!!
            В версии без исключения здесь я оставляла строку s=input()
            На семинаре input находится на месте tmp=s,но тогда ломается логика всей программы 
            Как переопределить s после исключения,если оно заыершает программу
            """
